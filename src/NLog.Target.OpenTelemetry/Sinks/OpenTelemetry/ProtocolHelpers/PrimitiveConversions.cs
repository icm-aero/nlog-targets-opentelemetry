...
static class PrimitiveConversions
{
    ...
    {
        if (level == LogLevel.Trace) return SeverityNumber.Trace;
        if (level == LogLevel.Debug) return SeverityNumber.Debug;
        if (level == LogLevel.Info) return SeverityNumber.Info;
        if (level == LogLevel.Warn) return SeverityNumber.Warn;
        if (level == LogLevel.Error) return SeverityNumber.Error;
        if (level == LogLevel.Fatal) return SeverityNumber.Fatal;
    ...
    public static AnyValue ToOpenTelemetryMap(DictionaryValue value)
    {
        ...
        var kvList = new KeyValueList();
        map.KvlistValue = kvList;
        foreach (var prop in value.Elements)
        {
            var v = ToOpenTelemetryAnyValue(prop.Value);
            var kv = new KeyValue
            {
                Key = ToOpenTelemetryAnyValue(prop.Key).StringValue,
                Value = v
            };
            kvList.Values.Add(kv);
        }
    ...
    public static AnyValue ToOpenTelemetryArray(DictionaryValue value)
    {
        ...
        foreach (var element in value.Elements)
        {
            ...
            {
                Key = ToOpenTelemetryAnyValue(element.Key).StringValue,
    ...
    internal static AnyValue ToOpenTelemetryAnyValue(LogEventPropertyValue value)
    {
        return value switch
        {
            ...
            DictionaryValue d => ToOpenTelemetryMap(d), //ToOpenTelemetryArray(d),
    ...
    internal static string OnlyHexDigits(string s)
    {
        ...
        {
            InternalLogger.Error("Regular expression matching timed out over {0}", s);
            return string.Empty;
        }
    }
