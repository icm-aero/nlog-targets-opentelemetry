...
partial class PropertyValueConverter: ILogEventPropertyFactory, ILogEventPropertyValueFactory
{
    ...
    public LogEventPropertyValue CreatePropertyValue(object? value, Destructuring destructuring)
    {
        ...
        {
            InternalLogger.Error(ex, "Exception caught while converting property value: {0}");
    ...
    IEnumerable<LogEventProperty> GetProperties(object value, [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicProperties)] Type type)
    {
        foreach (var prop in type.GetPropertiesRecursive())
        {
            ...
            catch (TargetParameterCountException)
            {
                ...
                InternalLogger.Error("The property accessor {0} is a non-default indexer", prop);
                continue;
            }
            catch (TargetInvocationException ex)
            {
                InternalLogger.Error(ex, "The property accessor {0} threw exception: {1}", prop);
            ...
            {
                InternalLogger.Error("The property accessor {0} is not supported via Reflection API", prop);
